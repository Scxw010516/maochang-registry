...
def get_parameters(image: IMAGE, piles: Sequence[List]) -> tuple[dict, dict]:
    ...
    # 面弯
    face_angle = 180 - (face_angle_up + face_angle_down) / 2
    ...
    # 镜腿外张角
    spread_split = [0.8, 0.75, 0.7, 0.6, 0.5, 0.4, 0.3]
    for i in spread_split:
        if w * (i + 0.02) < piles[0][0]:
            w_temp = int(w * i)
            spread_point_up = [
                w_temp,
                get_first_no_zero(crop[:, w_temp - 20 : w_temp + 20]),
            ]
            spread_point_down = [
                w_temp,
                get_first_no_zero(crop[:, w_temp - 20 : w_temp + 20][::-1]),
            ]
            spread_point_up[0] += x
            spread_point_up[1] += y
            spread_point_down[0] += x
            spread_point_down[1] = h - spread_point_down[1] + y
            spread_angle_up = get_angle(spread_point_up, piles[0]) * 180 / np.pi + 90
            spread_angle_down = (
                -get_angle(spread_point_down, piles[1]) * 180 / np.pi + 90
            )
            break
    else:
