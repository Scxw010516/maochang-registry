...
from ..blocks.helper import maybe_convert_scalar_to_list, get_matching_pool_op
from ..blocks.simple_conv_blocks import StackedConvBlocks
...
class PlainConvEncoder(nn.Module):
    def __init__(
        ...
        return_skips: bool = False,  # 跳跃连接，返回每一层结果
        ...
        for s in range(n_stages):
            ...
            if pool == "max" or pool == "avg":
                ...
                raise RuntimeError('pool must be "max", "avg" or "conv".')
            stage_modules.append(
                StackedConvBlocks(
                    n_conv_per_stage[s],
                    conv_op,
