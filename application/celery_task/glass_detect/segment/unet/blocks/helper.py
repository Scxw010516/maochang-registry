...
def convert_dim_to_conv_op(dimension: int) -> Type[_ConvNd]:
    """
    :param dimension: 1, 2 or 3 卷积维度
    :return: conv Class of corresponding dimension 对应维度的卷积层
    """
    if dimension == 1:
        return nn.Conv1d
    elif dimension == 2:
...
def get_default_network_config(
    ...
    if norm_type == "bn":  # batch norm
        config["norm_op"] = get_matching_batchnorm(dimension=dimension)
    elif norm_type == "in":  # instance norm
