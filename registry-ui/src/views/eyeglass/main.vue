<template>
  ...
  <a-layout v-show="store.state.hasLoggedIn" class="full-height-row">
    ...
    <a-layout-sider style="background-color: #8675ff" width="230px">
      ...
      >
        <a-menu-item
          ...
          :key="item.key"
          style="height: 60px; font-weight: 400; font-size: 20px"
        >
          <template #icon>
            <svg
    ...
    >
      <a-layout-content
        style="background-color: #ffffff; border-radius: 12px 0px 0px 12px"
      >
        <!-- 拍摄 -->
        <a-row
        ...
        >
          <a-col :span="8" class="capture-box-2">
            <img src="" alt="" class="capture-frame" />
          </a-col>
          <a-col :span="8" class="capture-box-2">
            <img src="" alt="" class="capture-frame" />
          </a-col>
        ...
        >
          <a-col :span="24" style="height: 100%; overflow: auto">
            ...
            <a-row style="margin-top: 18px">
              ...
              <a-col>
                ...
                >
                  ...
                  <a-col :span="6">
                    <a-button
                      :disabled="hasWeightLoged"
                      @click="onClickResetWeight"
                    >
                      去皮
                    </a-button>
                    <a-button
            ...
            >
              <a-row>
                <p
                  style="margin: 0px 15.5px; font-size: 30px; font-weight: bold"
                >
                  详细信息
                </p>
              </a-row>
...
import {
  ...
  // EyeGlassImageFormLabel, // 镜架图像背景参数接口
...
async function initCamera(): Promise<boolean> {
    ...
    .then(function (devices) {
      ...
      devices.forEach(function (device) {
        ...
        if (
          ...
          device.label.includes("16MP USB Camera (32e4:0006)")
        ) {
          // 添加设备到设备列表
  ...
  // 判断设备列表是否为空
  if (store.state.cameraState.cameraList.length === 0) {
    // 报告错误
  ...
  // 判断设备不足
  if (store.state.cameraState.cameraList.length < 3) {
    // 报告错误
    console.log("摄像头设备不足");
    // 重置设备列表
    initCameraDeviceState();
  ...
  // 设置摄像头设备初始化状态
  store.state.cameraState.cameraInitState = true;
  // 返回初始化成功
  return true;
}
...
async function initCameraDeviceState(): Promise<void> {
  // 重置摄像头设备初始化状态
  store.state.cameraState.cameraInitState = false;
  // 重置摄像头设备列表
  store.state.cameraState.cameraList = [];
}
...
const onClickCaptureOrConfirm = () => {
  ...
  switch (currentStage.value) {
    ...
    // break;
    case "preview": //预览
      if (store.state.cameraState.cameraInitState) {
        // 开启取流，进行预览
        stopCameraStream()
          .then(() => {
            currentStage.value = "confirm";
...
const onClickReturnOrRedo = () => {
  switch (currentStage.value) {
    ...
    //   currentStage.value = "confirm-2";
    //   break;
    case "preview": // 预览：返回至输入基础信息input-basic-params
      currentStage.value = "input-basic-params";
      // 关闭所有视频流
      store.state.cameraState.cameraList.forEach((camera) => {
        if (camera.mediaStream) {
      ...
      break;
    case "confirm": // 确认：返回至拍摄预览preview
      currentStage.value = "preview";
      startCameraStream();
      break;
    case "input-params": // 计算参数：返回至确认confirm
      currentStage.value = "confirm";
      // 关闭read-weight的WebSocket
      wsMap.value.get("weight")?.close(1000, "客户端关闭read-weight");
      currentStage.value = "confirm";
