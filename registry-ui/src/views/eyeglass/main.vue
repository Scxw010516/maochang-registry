<template>
  <a-row v-if="!store.state.hasLoggedIn" class="full-height-row" type="flex" align="middle" justify="center">
    <!-- 登录 输入仓库地址 -->
    <a-form v-if="inputAddress" class="login-form-card">
      <LeftOutlined @click="inputAddress = false" />
      <a-select v-model:value="store.state.warehouse" placeholder="选择仓库地址" style="width: 100%" show-search defaultOpen
        :options="store.state.options.warehouse_options" :filter-option="filterOptionbyLabel"
        @select="inputAddress = false"></a-select>
    </a-form>
    <!-- 登录 输入员工号 -->
    <a-form v-else class="login-form-card">
      <img src="@/assets/logo.png" alt="logo" />
      <a-form-item name="username" style="margin-bottom: 24px; margin-top: 48px">
        <a-input class="login-input" allow-clear size="large" v-model:value="store.state.username"
          :placeholder="'请输入员工号'">
        </a-input>
      </a-form-item>
      <a-form-item>
        <a-button class="login-button" block size="large" type="primary" @click="onClickLogin">
          登录
        </a-button>
      </a-form-item>
      <!-- 展示仓库地址 -->
      <a-form-item name="colllection-address" style="margin-bottom: 24px">
        <a-button class="login-address" type="link" @click="inputAddress = true">
  ...
  <a-layout v-if="store.state.hasLoggedIn" class="full-height-row">
    ...
    <a-layout-sider style="background-color: #8675ff" width="230px">
      ...
      <!-- 目录 -->
      <a-menu style="background-color: #8675ff" mode="inline" theme="dark" v-model:selectedKeys="selectedMenuItem">
        <a-menu-item v-for="item in menuItems" :key="item.key" style="height: 60px; font-weight: 400; font-size: 20px">
          <template #icon>
            <svg v-if="selectedMenuItem.includes(item.key)" width="20" height="20" viewBox="0 0 20 20" fill="none"
    ...
    " class="full-height-row">
      <a-layout-content>
        <!-- 输入SKU -->
        <a-row v-if="currentStage === 'input-sku'" style="
            height: 100vh;
            width: 100%;
          ...
          <a-col class="sku-form-card">
            <!-- <a-row>
              <a-form
                ...
                labelAlign="left"
              >
                <a-form-item name="sku" label="sku">
                  <a-input
                    v-model:value="EyeGlassBasicFormState.sku"
                    :bordered="false"
                  ></a-input>
                </a-form-item>
                <a-form-item name="model_type" label="型号">
                  <a-input
            ...
            <a-row>
              <a-select v-model:value="store.state.skuormodeltype" placeholder="选择检索类型" placement="topLeft"
                style="width: 70px; margin-bottom: 20px" :options="store.state.options.skuormodeltype_options"
            ...
            <a-row style="
              ...
              <a-auto-complete v-model:value="searchString" :options="searchOptions" :dropdownMatchSelectWidth="584"
                style="width: 500px" :defaultActiveFirstOption="false" @select="onSelectModeltypeOrSKU"
                @search="onSearchModeltypeOrSKU">
                <template #option="item">
                  <div v-if="store.state.skuormodeltype == 1">
                    <a-row :gutter="2">
                      <a-col :span="9" class="wrap-content">型号：{{ item.model_type }}</a-col>
                      <a-col :span="9" class="wrap-content">SKU：{{ item.sku }}</a-col>
                      <a-col :span="6" class="wrap-content">品牌：{{ item.brand }}</a-col>
                    </a-row>
                  </div>
                ...
                <a-input v-model:value="searchString" allow-clear :placeholder="store.state.skuormodeltype === 1
                  ? '请输入镜架型号'
                  : '请输入镜架SKU'
                  " @keyup.enter="onClickEnterSKU" />
              </a-auto-complete>
              <a-button autofocus type="primary" @click="onClickEnterSKU" @keyup.enter="onClickEnterSKU">
                确认
              </a-button>
            </a-row>
        ...
        <!-- 输入基础信息 -->
        <a-row v-else align="middle" justify="center" style="
            height: 100vh;
            width: 100%;
            background-color: #ffffff;
            border-radius: 12px 0px 0px 12px;
          ...
          <a-col class="input-basic-form-card">
            <a-form ref="EyeGlassBasicFormRef" :model="EyeGlassBasicFormState" :rules="EyeGlassBasicFormRules"
              hideRequiredMark :label-col="{ span: 7 }" :wrapper-col="{ span: 17 }" labelAlign="left">
              <a-form-item name="sku">
                <a-input class="sku-input" v-model:value="EyeGlassBasicFormState.sku" disabled></a-input>
              </a-form-item>
              <div style="height: 550px; overflow: auto">
                <a-form-item label="品牌" name="brand" class="basic-item">
                  <a-auto-complete v-model:value="EyeGlassBasicFormState.brand"
                    :options="store.state.options.brand_options" :filter-option="filterOptionbyValue"
                    @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item label="型号" name="model_type" class="basic-item">
                  <a-auto-complete v-model:value="EyeGlassBasicFormState.model_type"
                    :options="store.state.options.model_type_options" :filter-option="filterOptionbyValue"
                    @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item label="价格" name="price" class="basic-item">
                  <a-input-number v-model:value="EyeGlassBasicFormState.price" :precision="2" :controls="false"
                    decimalSeparator="." @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item label="材质" name="material" class="basic-item">
                  <a-select v-model:value="EyeGlassBasicFormState.material" placeholder="选择材质（单选）"
                    :options="store.state.options.material_options" @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item label="颜色" name="color" class="basic-item">
                  <a-select v-model:value="EyeGlassBasicFormState.color" placeholder="选择颜色（单选）"
                    :options="store.state.options.color_options" @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item label="形状" name="shape" class="basic-item">
                  <a-select v-model:value="EyeGlassBasicFormState.shape" placeholder="选择形状（单选）"
                    :options="store.state.options.shape_options" @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item label="鼻托" name="isnosepad" class="basic-item">
                  <a-radio-group v-model:value="EyeGlassBasicFormState.isnosepad"
                    @keyup.enter="onClickEnterBasicParams">
                    <a-radio-button :value="1" style="
                        border-top-left-radius: 9px;
                        border-bottom-left-radius: 9px;
                      ">有
                    </a-radio-button>
                    <a-radio-button :value="0" style="
                ...
                <a-form-item label="库存" name="stock" class="basic-item">
                  <a-input-number v-model:value="EyeGlassBasicFormState.stock" min="0" :controls="false"
                    @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item label="撑片弧度" name="lens_radian" class="basic-item">
                  <a-input-number v-model:value="EyeGlassBasicFormState.lens_radian" :controls="false"
                    @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item v-show="false" label="镜片宽度" name="lens_width_st" class="basic-item">
                  <a-input-number v-model:value="EyeGlassBasicFormState.lens_width_st" :controls="false"
                    @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item v-show="false" label="鼻梁宽度" name="bridge_width_st" class="basic-item">
                  <a-input-number v-model:value="EyeGlassBasicFormState.bridge_width_st" :controls="false"
                    @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
                <a-form-item v-show="false" label="镜腿长度" name="temple_length_st" class="basic-item">
                  <a-input-number v-model:value="EyeGlassBasicFormState.temple_length_st" :controls="false"
                    @keyup.enter="onClickEnterBasicParams" />
                </a-form-item>
              </div>
            ...
            <a-button style="
                ...
                color: #ffffff;
                font-size: 18px;
              " @click="onClickEnterBasicParams" @keyup.enter="onClickEnterBasicParams">
              开始拍摄
            </a-button>
            <a-button style="
                margin-top: 28px;
    ...
    " class="full-height-row">
      <a-layout-content style="background-color: #ffffff; border-radius: 12px 0px 0px 12px">
        <!-- 拍摄 -->
        <a-row v-if="currentStage !== 'input-params'" class="registry-preview" type="flex" align="middle"
          justify="center">
          ...
          <div class="steps-line" style="position: absolute; bottom: 140px">
            <a-steps label-placement="vertical">
              <a-step v-for="(item, index) in stepItems" :key="item" :title="item.title" :status="index < 2 &&
                item.status == 'finish' &&
                ...
                <template #icon>
                  <!-- 白色 -->
                  <svg v-if="currentStage.includes(index.toString())" width="37" height="37" viewBox="0 0 37 37"
                    fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="18.5" cy="18.5" r="9.25" transform="rotate(90 18.5 18.5)" fill="white" />
                    <circle cx="18.5" cy="18.5" r="17.3438" transform="rotate(90 18.5 18.5)" stroke="white"
                      stroke-width="2.3125" stroke-dasharray="4.62 4.62" />
                  </svg>
                  <!-- 紫色 -->
                  <svg v-else-if="item.status === 'finish'" width="37" height="37" viewBox="0 0 37 37" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <circle cx="18.5" cy="18.5" r="9.25" transform="rotate(90 18.5 18.5)" fill="#8675FF" />
                    <circle cx="18.5" cy="18.5" r="17.3438" transform="rotate(90 18.5 18.5)" stroke="#8675FF"
                      stroke-width="2.3125" stroke-dasharray="4.62 4.62" />
                  </svg>

                  <!-- 灰色 -->
                  <svg v-else-if="item.status === 'wait'" width="37" height="37" viewBox="0 0 37 37" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <circle cx="18.5" cy="18.5" r="9.25" transform="rotate(90 18.5 18.5)" fill="#B4B4B4" />
                    <circle cx="18.5" cy="18.5" r="17.3438" transform="rotate(90 18.5 18.5)" stroke="#999999"
                      stroke-width="2.3125" stroke-dasharray="4.62 4.62" />
                  </svg>
                </template>
              </a-step>
        ...
        <!-- 计算参数 -->
        <a-row v-else class="registry-preview" type="flex" align="middle" justify="center">
          <a-col :span="24" style="height: 100%; overflow: auto">
            ...
            <a-row style="margin-top: 18px">
              ...
              <a-col :span="24">
                ...
                <!-- 录入的基础信息 -->
                <a-form ref="EyeGlassBasicFormRef" :model="EyeGlassBasicFormState" :rules="EyeGlassBasicFormRules"
                  hideRequiredMark layout="inline" autocomplete="off">
                  <a-row :gutter="[26, 32]" style="
                      margin-left: 40px !important;
                      width: calc(100% - 85px);
                    ">
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="SKU" name="sku" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-input v-model:value="EyeGlassBasicFormState.sku" disabled></a-input>
                      </a-form-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="品牌" name="brand" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-auto-complete v-model:value="EyeGlassBasicFormState.brand"
                          :options="store.state.options.brand_options" :filter-option="filterOptionbyValue" />
                      </a-form-item>
                    ...
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="型号" name="model_type" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-auto-complete v-model:value="EyeGlassBasicFormState.model_type"
                          :options="store.state.options.model_type_options" :filter-option="filterOptionbyValue" />
                      </a-form-item>
                    ...
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="价格" name="price" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-input-number v-model:value="EyeGlassBasicFormState.price" :precision="2" :controls="false"
                          decimalSeparator="." />
                      </a-form-item>
                    ...
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="材质" name="material" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-select v-model:value="EyeGlassBasicFormState.material" placeholder="选择材质（单选）"
                    ...
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="颜色" name="color" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-select v-model:value="EyeGlassBasicFormState.color" placeholder="选择颜色（单选）"
                    ...
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="形状" name="shape" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-select v-model:value="EyeGlassBasicFormState.shape" placeholder="选择形状（单选）"
                    ...
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="鼻托" name="isnosepad" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-radio-group v-model:value="EyeGlassBasicFormState.isnosepad">
                          <a-radio-button :value="1" style="
                              border-top-left-radius: 9px;
                    ...
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="库存" name="stock" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-input-number class="calculate-input" v-model:value="EyeGlassBasicFormState.stock" min="0"
                          :controls="false" />
                      </a-form-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-item class="calculate-item" label="撑片弧度" name="lens_radian" :labelCol="{ span: 6 }"
                        :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-input-number class="calculate-input" v-model:value="EyeGlassBasicFormState.lens_radian"
                          :controls="false" />
                      </a-form-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-item v-show="false" class="calculate-item" label="镜片宽度" name="lens_width_st"
                        :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-input-number class="calculate-input" v-model:value="EyeGlassBasicFormState.lens_width_st"
                          :controls="false" />
                      </a-form-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-item v-show="false" class="calculate-item" label="鼻梁宽度" name="bridge_width_st"
                        :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-input-number class="calculate-input" v-model:value="EyeGlassBasicFormState.bridge_width_st"
                          :controls="false" />
                      </a-form-item>
                    </a-col>
                    <a-col :span="6">
                      <a-form-item v-show="false" class="calculate-item" label="镜腿长度" name="temple_length_st"
                        :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }" labelAlign="left">
                        <a-input-number class="calculate-input" v-model:value="EyeGlassBasicFormState.temple_length_st
                          " :controls="false" />
                      </a-form-item>
                    </a-col>
                    <!-- <a-col :span="6">
              ...
              <a-col>
                ...
                <p class="calculate-title">
                  详细信息
                  <MenuUnfoldOutlined style="margin-left: 5px; font-size: 20px; color: #666666"
                ...
                <a-row :gutter="[26, 32]" style="
                  ...
                  <a-col :span="6">
                    <!-- 计算参数：镜架重量 -->
                    <a-form ref="EyeGlassWeightFormRef" :model="EyeGlassWeightFormState"
                      :rules="EyeGlassWeightFormRules" hideRequiredMark layout="inline" autocomplete="off"
                      :labelCol="{ span: 10 }" :wrapperCol="{ span: 14 }" labelAlign="left">
                      <a-form-item class="calculate-item" label="称重结果" name="weight">
                        <a-input v-model:value="EyeGlassWeightFormState.weight" :suffix="EyeGlassWeightFormUnit.weight"
                          disabled></a-input>
                      </a-form-item>
                    </a-form>
                  </a-col>
                  ...
                  <a-col :span="6">
                    <!-- 镜架风格参数 -->
                    <a-form ref="EyeGlassStyleFormRef" :model="EyeGlassStyleFormState" :rules="EyeGlassStyleFormRules"
                      hideRequiredMark layout="inline" autocomplete="off" :labelCol="{ span: 10 }"
                ...
                <!-- 计算参数：详细信息 -->
                <a-form ref="EyeGlassDetailFormRef" :model="EyeGlassDetailFormState" :rules="EyeGlassDetailFormRules"
                  hideRequiredMark layout="inline" autocomplete="off" :labelCol="{ span: 10 }"
                  :wrapperCol="{ span: 14 }" labelAlign="left">
                  <a-row :gutter="[26, 32]" style="
                      margin-left: 40px !important;
                      width: calc(100% - 85px);
                    ">
                    <a-col :span="6" v-for="(value, key) in EyeGlassDetailToviewFormLabel" :key="key">
                      <a-form-item :label="value" class="calculate-item" :name="key">
                        <a-input v-model:value="EyeGlassDetailFormState[key]" :suffix="EyeGlassDetailFormUnit[key]"
                          disabled></a-input>
                      </a-form-item>
                    </a-col>
                  </a-row>
            ...
            <!-- 详细信息modal -->
            <a-modal width="calc(360*3px + 80*2px + 44*2px)" v-model:open="showDetailModal" :footer="null"
              ...
              <a-form ref="EyeGlassDetailModelFormRef" :model="EyeGlassDetailModelFormState"
                :rules="EyeGlassDetailFormRules" hideRequiredMark layout="inline" autocomplete="off"
                :labelCol="{ span: 10 }" :wrapperCol="{ span: 14 }" labelAlign="left">
                <a-row :gutter="[80, 20]" style="margin: 24px 20px">
                  <a-col :span="8" v-for="(value, key) in EyeGlassDetailFormLabel" :key="key">
                    <a-form-item :label="value" class="modal-item" :name="key">
                      <a-input class="modal-input" v-model:value="EyeGlassDetailModelFormState[key]" :disabled="isInputEditable !== EyeGlassDetailFormLabel[key]
                        " :suffix="EyeGlassDetailFormUnit[key]">
                        <template #suffix>
                          <FormOutlined style="color: rgba(0, 0, 0, 0.45)" @click="
                            isInputEditable = EyeGlassDetailFormLabel[key]
                            " />
                        </template>
                      </a-input>
                    </a-form-item>
        ...
        <!-- 操作按钮 -->
        <a-row class="registry-operations" type="flex" align="middle" justify="end"
    ...
    <!-- 镜框管理 -->
    <a-layout class="full-height-row" v-if="Array.isArray(selectedMenuItem) && selectedMenuItem.includes('2')">
      <a-layout-content>
        <ManagePage />
      </a-layout-content>
    </a-layout>
    <!-- 账号中心 -->
    <a-layout class="full-height-row" v-if="Array.isArray(selectedMenuItem) && selectedMenuItem.includes('3')">
      <a-layout-content>
  ...
  <!-- 摄像头未启动提示Modal -->
  <a-modal v-model:open="showCameraStateErrorModal" centered title="是否尝试重启摄像头?" :okText="'重启'" :cancelText="'取消'"
    @ok="onClickCameraStateErrorOk" :confirmLoading="cameraStateErrorModalLoading" :maskClosable="false"
    :keyboard="false">
    <p>摄像头未能正常启动，请尝试重新插拔设备并重启</p>
  </a-modal>
  <!-- 秤未启动的提示Modal -->
  <a-modal v-model:open="showWeightStateErrorModal" centered title="是否尝试重启电子秤？" :okText="'重启'" :cancelText="'取消'"
    @ok="onClickWeightStateErrorOk" :confirmLoading="weightStateErrorModalLoading" :maskClosable="false"
    :keyboard="false">
    <p>电子秤未能正常启动，请尝试重启</p>
  </a-modal>
</template>
...
<script lang="ts" setup>
  ...
  import {
    ...
    // EyeGlassBasicFormUnit, // 镜架基础参数单位标签
  ...
  import { initFormOptions, getAllWarehouses } from "./utils";
  ...
  // const hasLoggedIn = ref<boolean>(); // 登录标识
  ...
  // //仓库地址选项
  // const collectionAddressOptions: { value: string; disabled: boolean }[] = [];
  ...
  // 镜架图像参数表单实例
  // const EyeGlassImageFormRef = ref()
  // 镜架图像参数表单初始化数据
  ...
  // 镜架计算参数表单实例
  // const EyeGlassCalculateParamsRef = ref()
  // 镜架计算参数表单数据
  // const EyeGlassCalculateParamsState: UnwrapRef<EyeGlassCalculateParams> =
  //   reactive({
  //     num_iter_0: "8",
  //     num_iter_1: "4",
  //     num_iter_2: "200",
  ...
  onMounted(() => {
    ...
    //   var collection = storage.getItem("warehouse");
    //   if (collection) {
    //     store.state.warehouse = collection;
    //   } else {
  ...
  const onClickLogin = () => {
    ...
    // if (window.localStorage) {
    //   if (store.state.warehouse != "") {
    //     window.localStorage.setItem("warehouse", store.state.warehouse);
    //     console.log(window.localStorage.getItem("warehouse"));
    //   } else {
